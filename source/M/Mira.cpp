	...
	uintptr_t GetBaseAddress(const char* path)
	{
		...
		for (auto l_Module : g_processmodulelist)
		{
			if (strcasecmp(l_Module.Path, path) == 0)
			{
				...
				{
					OrbisMessageHandler::KernelPrintOut("va: (%p) textsz: (%p) data: (%p), datasz: (%p), path: (%s), pltgot: (%p), entry: (%p) realloc: (%p)", l_Module.VirtualAddressBase, l_Module.TextSize, l_Module.DataBase, l_Module.DataSize, l_Module.Path, l_Module.PltGot, l_Module.Entry, l_Module.ReallocBase);
				}
