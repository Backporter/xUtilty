...
#include "../include/DialogueManger.h"
...
namespace xUtilty
{
	namespace DialogueManger
	{
		...
		int32_t OrbisDialogueManger::ShowMsgDialog()
		{
			...
			{
				Notify("sceSysmoduleLoadModule(0x00a4) failed with 0x%lx", ret);
				return ret;
			}

			if ((ret = sceCommonDialogInitialize()) < 0 && ret != 0x80B80002)
			{
				Notify("sceCommonDialogInitialize() failed with 0x%lx", ret);
				return ret;
			}

			if ((ret = sceMsgDialogInitialize()) < 0 && ret != 0x80B80004)
			{
				Notify("sceMsgDialogInitialize() failed with 0x%lx", ret);
				return ret;
			}

			if ((ret = sceMsgDialogOpen(&MsgDialogParam)) < 0)
			{
				Notify("sceMsgDialogOpen failed with 0x%lx", ret);
				sceMsgDialogClose();
				return ret;
			}
		...
		int32_t OrbisDialogueManger::ShowErrorDialog()
		{
			...
			{
				Notify("sceSysmoduleLoadModule(0x00ac) failed with 0x%lx", ret);
				return ret;
			}
			...
			{
				Notify("sceErrorDialogInitialize() failed with 0x%lx", ret);
				return ret;
			}

			if ((ret = sceErrorDialogOpen(&ErrorDialogParam)) < 0)
			{
				Notify("sceErrorDialogOpen failed with 0x%lx", ret);
				sceMsgDialogClose();
				return ret;
			}
		...
		int32_t OrbisDialogueManger::ShowImeDialog(char* buf)
		{
			...
			{
				Notify("sceSysmoduleLoadModule(0x0096) failed with 0x%lx", ret);
				return ret;
			}
			...
			{
				Notify("sceImeDialogInit() failed with 0x%lx", ret);
				sceSysmoduleUnloadModule(0x0096);
				return ret;
			}
			...
			{
				Notify("sceImeDialogGetResult(&ImeDialogResult) failed with 0x%lx", ret);
				sceSysmoduleUnloadModule(0x0096);
				return ret;
			}
			...
			{
				Notify("sceImeDialogTerm() failed with 0x%lx", ret);
				sceSysmoduleUnloadModule(0x0096);
				return ret;
			}
