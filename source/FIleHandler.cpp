...
namespace xUtilty
{
	namespace FileSystem
	{
			...
			m_flags(a_flags), m_filepath(0), m_fd(0), m_ret(0), m_fdpos(0)
		{
			...
			{
				KernelPrintOut("%s Failed to open %s for 0x%lx(%s)", __FUNCTION__, m_filepath, m_fd, strerror(errno));
				return;
			}

			if ((m_ret = fstat(m_fd, &m_filestats)) != 0)
			{
				KernelPrintOut("%s failed to gather file stats for 0x%lx", __FUNCTION__, m_ret);
				return;
			}
		}
		...
		FIleHandler::~FIleHandler()
		{
			if ((m_ret = close(m_fd)) != 0)
				KernelPrintOut("%s Failed to close file(%d/%s) for 0x%lx(%s)", __FUNCTION__, m_fd, m_filepath, m_ret, strerror(errno));
		...
		bool	 FIleHandler::Rewind(int64_t count)
		{
		...
		uint64_t FIleHandler::WriteBytes(void* src, size_t srcLength)
		{
			...
			KernelPrintOut("%s failed with 0x%lx(%s)", __FUNCTION__, m_ret, strerror(errno));
			return -1;
		}
